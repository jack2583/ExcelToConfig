...
public partial class TableExportToHrlHelper
{
    private static string _GetOneField(FieldInfo fieldInfo, int row,  out string errorString)
    {
        ...
        // 变量名前的缩进
       // content.Append(_GetHrlIndentation(level));
        // 变量名
       ...
       // content.Append(" = ");
        ...
        switch (fieldInfo.DataType)
        {
            ...
            case DataType.Float:
                {
                    errorString = string.Format("_GetOneField函数中未定义{0}类型数据导出至Hrl文件的形式", fieldInfo.DataType);
    ...
    private static string _GetDateValue(FieldInfo fieldInfo, int row, int level)
    {
        ...
        switch (dateFormatType)
        {
            ...
            default:
                {
                    AppLog.LogErrorAndExit("错误：用_GetDateValue函数导出Hrl文件的date型的DateFormatType非法");
                    break;
                }
        }
    ...
    private static string _GetTimeValue(FieldInfo fieldInfo, int row, int level)
    {
        ...
        switch (timeFormatType)
        {
            ...
            default:
                {
                    AppLog.LogErrorAndExit("错误：用_GetTimeValue函数导出Hrl文件的time型的TimeFormatType非法");
                    break;
                }
        }
    ...
    private static string _GetHrlIndentation(int level)
    {
        StringBuilder stringBuilder = new StringBuilder();
        for (int i = 0; i < level; ++i)
            stringBuilder.Append(" ");// 用于缩进lua table的字符串 tab
