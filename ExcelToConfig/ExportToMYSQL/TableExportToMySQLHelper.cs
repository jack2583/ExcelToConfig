        ...
        if (count > 0)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                foreach (FieldInfo fieldInfo in allDatabaseFieldInfo)
                {
                    ...
                    {
                        string toDatabaseFormatDefine = fieldInfo.ExtraParam[MySQLStruct.DateToExportFormatKey].ToString();
                        ...
                        if (toDatabaseFormatType == DateFormatType.FormatString)
                        {
                            ...
                            if (fieldInfo.DatabaseFieldType.Equals("date", StringComparison.CurrentCultureIgnoreCase))
                                values.Add(string.Format("'{0}'", ((DateTime)(fieldInfo.Data[i])).ToString(MySQLStruct.APP_DEFAULT_ONLY_DATE_FORMAT)));
                            ...
                            // date型导出到MySQL中的其他数据类型字段如varchar，采用声明的指定格式
                                ...
                                values.Add(string.Format("'{0}'", ((DateTime)(fieldInfo.Data[i])).ToString(fieldInfo.ExtraParam[MySQLStruct.DateToExportFormatKey].ToString())));
                        }
                    ...
                    {
                        string toDatabaseFormatDefine = fieldInfo.ExtraParam[MySQLStruct.TimeToExportFormatKey].ToString();
                        ...
                        if (toDatabaseFormatType == TimeFormatType.FormatString)
                        {
                            if (fieldInfo.DatabaseFieldType.StartsWith("time", StringComparison.CurrentCultureIgnoreCase))
                                ...
                                values.Add(string.Format("'{0}'", ((DateTime)(fieldInfo.Data[i])).ToString(fieldInfo.ExtraParam[MySQLStruct.TimeToExportFormatKey].ToString())));
                        }
