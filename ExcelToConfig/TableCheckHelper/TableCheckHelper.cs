...
public partial class TableCheckHelper
{
            ...
            foreach (object value in effectiveValues[currentLevel])
            {
                if (!inputData.Contains(value))
                {
                    if (currentLevel > 0)
                    {
                        ...
                        errorStringBuilder.AppendFormat("字段\"{0}\"（列号：{1}）缺少在{2}情况下值为\"{3}\"的数据\n", indexField[currentLevel].FieldName, ExcelMethods.GetExcelColumnName(indexField[currentLevel].ColumnSeq + 1), parentKeyInfo, value);
                    }
                    else
                        errorStringBuilder.AppendFormat("字段\"{0}\"（列号：{1}）缺少值为\"{2}\"的数据\n", indexField[currentLevel].FieldName, ExcelMethods.GetExcelColumnName(indexField[currentLevel].ColumnSeq + 1), value);
                }
            }
        }
    ...
    private static bool CheckOneField(FieldInfo fieldInfo, out string errorString)
    {
        ...
        if (errorString != null)
        {
            errorStringBuilder.AppendFormat("字段\"{0}\"（列号：{1}）填写的检查规则\"{2}\"不合法：{3}，不得不跳过对该字段的检查\n\n", fieldInfo.FieldName, ExcelMethods.GetExcelColumnName(fieldInfo.ColumnSeq + 1), fieldInfo.CheckRule, errorString);
            errorString = null;
        }
    ...
    public static bool CheckByRules(List<FieldCheckRule> checkRules, FieldInfo fieldInfo, out string errorString)
    {
        ...
        foreach (FieldCheckRule checkRule in checkRules)
        {
            switch (checkRule.CheckType)
            {
                ...
                default:
                    {
                        AppLog.LogErrorAndExit(string.Format("用CheckTable函数解析出了检查规则，但没有对应的检查函数，检查规则类型为{0}", checkRule.CheckType));
                        break;
                    }
            }
            ...
            if (errorString != null)
            {
                errorStingBuilder.AppendFormat("字段\"{0}\"（列号：{1}）未通过\"{2}\"的检查规则\n{3}\n", fieldInfo.FieldName, ExcelMethods.GetExcelColumnName(fieldInfo.ColumnSeq + 1), checkRule.CheckRuleString, errorString);
                errorString = null;
            }
        }
