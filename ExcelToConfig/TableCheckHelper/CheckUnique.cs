        ...
        if (repeatedDataInfo.Count > 0)
        {
            ...
            foreach (var item in repeatedDataInfo)
            {
                if (dataType == DataType.Date)
                    repeatedLineInfo.AppendFormat("数据\"{0}\"重复，重复的行号为：", ((DateTime)(item.Key)).ToString(DateTimeValue.APP_DEFAULT_DATE_FORMAT));
                if (dataType == DataType.Time)
                    repeatedLineInfo.AppendFormat("数据\"{0}\"重复，重复的行号为：", ((DateTime)(item.Key)).ToString(DateTimeValue.APP_DEFAULT_TIME_FORMAT));
                else
                    repeatedLineInfo.AppendFormat("数据\"{0}\"重复，重复的行号为：", item.Key);

                List<int> lineIndex = item.Value;
                foreach (int lineNum in lineIndex)
                    repeatedLineInfo.Append(lineNum + ExcelTableSetting.DataFieldDataStartRowIndex + 1 + ", ");
