        ...
        if (dataType == DataType.Int || dataType == DataType.Long || dataType == DataType.Float || dataType == DataType.Date || dataType == DataType.Time)
        {
            ...
            else if (dataType == DataType.Date)
            {
                ...
                for (int i = 0; i < values.Length; ++i)
                {
                    ...
                    {
                        errorString = string.Format("集合值定义中出现了非法的date型数据，其为\"{0}\"，请按{1}的形式填写", oneValueString, DateTimeValue.APP_DEFAULT_DATE_FORMAT);
                        return;
                    }
                }
            ...
            else if (dataType == DataType.Time)
            {
                ...
                for (int i = 0; i < values.Length; ++i)
                {
                    ...
                    try
                    {
                        ...
                        DateTime oneValue = new DateTime(DateTimeValue.REFERENCE_DATE.Year, DateTimeValue.REFERENCE_DATE.Month, DateTimeValue.REFERENCE_DATE.Day, tempDateTime.Hour, tempDateTime.Minute, tempDateTime.Second);
                        if (setValues.ContainsKey(oneValue))
                            repeatedSetValue.Add(oneValue);
                        else
                            setValues.Add(oneValue, true);
                    ...
                    {
                        errorString = string.Format("集合值定义中出现了非法的time型数据，其为\"{0}\"，请按{1}的形式填写", oneValueString, DateTimeValue.APP_DEFAULT_TIME_FORMAT);
                        return;
                    }
                }
