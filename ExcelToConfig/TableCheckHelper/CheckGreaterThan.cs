            ...
            if (isContainsEqual == true)
            {
                for (int i = 0; i < fieldInfo.Data.Count; ++i)
                {
                    ...
                    if (fieldDataValue < comparedFieldDataValue)
                        illegalValue.Add(new object[3] { i + ExcelTableSetting.DataFieldDataStartRowIndex + 1, fieldInfo.Data[i], comparedField.Data[i] });
                }
            }
            else
            {
        ...
        else if (fieldInfo.DataType == DataType.Date)
        {
            if (isContainsEqual == true)
            {
                for (int i = 0; i < fieldInfo.Data.Count; ++i)
                {
                    ...
                    if (fieldDataValue < comparedFieldDataValue)
                        illegalValue.Add(new object[3] { i + ExcelTableSetting.DataFieldDataStartRowIndex + 1, fieldDataValue.ToString(DateTimeValue.APP_DEFAULT_DATE_FORMAT), comparedFieldDataValue.ToString(DateTimeValue.APP_DEFAULT_DATE_FORMAT) });
                }
            }
            else
            {
        ...
        else if (fieldInfo.DataType == DataType.Time)
        {
            if (isContainsEqual == true)
            {
                for (int i = 0; i < fieldInfo.Data.Count; ++i)
                {
                    ...
                    DateTime comparedFieldDataValue = (DateTime)comparedField.Data[i];
                    if (fieldDataValue < comparedFieldDataValue)
                        illegalValue.Add(new object[3] { i + ExcelTableSetting.DataFieldDataStartRowIndex + 1, fieldDataValue.ToString(DateTimeValue.APP_DEFAULT_TIME_FORMAT), comparedFieldDataValue.ToString(DateTimeValue.APP_DEFAULT_TIME_FORMAT) });
                }
            }
            else
            {
