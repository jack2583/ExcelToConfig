...
public partial class TableCheckHelper
{
    ...
    public static bool CheckRange(FieldInfo fieldInfo, FieldCheckRule checkRule, out string errorString)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        DateTimeFormatInfo dateTimeFormat = new DateTimeFormatInfo();
                        dateTimeFormat.ShortTimePattern = DateTimeValue.APP_DEFAULT_TIME_FORMAT;
                        // 此函数会将DateTime的日期部分自动赋值为当前时间
                        DateTime tempDateTime = Convert.ToDateTime(floorString, dateTimeFormat);
                        floorDateTime = new DateTime(DateTimeValue.REFERENCE_DATE.Year, DateTimeValue.REFERENCE_DATE.Month, DateTimeValue.REFERENCE_DATE.Day, tempDateTime.Hour, tempDateTime.Minute, tempDateTime.Second);
                    }
                    catch
                    {
                        errorString = string.Format("值范围检查定义错误：time型下限声明不合法，必须按{0}的形式填写，你输入的为{1}\n", DateTimeValue.APP_DEFAULT_TIME_FORMAT, floorString);
                        return false;
                    }
                }
            }
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        DateTimeFormatInfo dateTimeFormat = new DateTimeFormatInfo();
                        dateTimeFormat.ShortDatePattern = DateTimeValue.APP_DEFAULT_TIME_FORMAT;
                        // 此函数会将DateTime的日期部分自动赋值为当前时间
                        DateTime tempDateTime = Convert.ToDateTime(ceilString, dateTimeFormat);
                        ceilDateTime = new DateTime(DateTimeValue.REFERENCE_DATE.Year, DateTimeValue.REFERENCE_DATE.Month, DateTimeValue.REFERENCE_DATE.Day, tempDateTime.Hour, tempDateTime.Minute, tempDateTime.Second);
                    }
                    catch
                    {
                        errorString = string.Format("值范围检查定义错误：time型上限声明不合法，必须按{0}的形式填写，你输入的为{1}\n", DateTimeValue.APP_DEFAULT_TIME_FORMAT, ceilString);
                        return false;
                    }
                }
            }
        ...
        else
        {
            ...
            {
                errorString = string.Format("值范围检查定义错误：上限值必须大于下限值，你输入的下限为{0}，上限为{1}\n", floorDateTime.ToString(DateTimeValue.APP_DEFAULT_DATE_FORMAT), ceilDateTime.ToString(DateTimeValue.APP_DEFAULT_DATE_FORMAT));
                return false;
            }
        }
