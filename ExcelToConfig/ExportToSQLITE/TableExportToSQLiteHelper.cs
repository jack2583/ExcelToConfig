...
public class TableExportToSQLiteHelper
{
    ...
    public static bool ExportTableToDatabase(TableInfo tableInfo, out string errorString)
    {
        ...
        if (tableInfo.TableConfig != null && tableInfo.TableConfig.ContainsKey(SQLiteStruct.CONFIG_NAME_EXPORT_DATABASE_TABLE_NAME))
        {
            ...
            {
                AppLog.LogWarning("警告：以下字段未设置导出SQLite数据库的信息，将被忽略：");
    ...
    private static bool _CreateTable(string tableName, TableInfo tableInfo, string comment, out string errorString)
    {
        ...
        foreach (FieldInfo fieldInfo in GetAllDatabaseFieldInfo(tableInfo))
        {
            ...
            if (fieldInfo.DataType == DataType.Date)
            {
                ...
                if (fieldInfo.DatabaseFieldType.StartsWith("time", StringComparison.CurrentCultureIgnoreCase))
                {
                    errorString = string.Format("date型字段\"{0}\"（列号：{1}）声明导出到SQLite中的数据类型错误，不允许为time型，如果仅需要时分秒部分，请在Excel中将该字段在本工具中的数据类型改为time型", fieldInfo.FieldName, ExcelMethods.GetExcelColumnName(fieldInfo.ColumnSeq + 1));
                    return false;
                }
                if (toDatabaseFormatType == DateFormatType.ReferenceDateSec || toDatabaseFormatType == DateFormatType.ReferenceDateMsec)
                {
                    ...
                    {
                        errorString = string.Format("date型字段\"{0}\"（列号：{1}）声明导出到SQLite中的形式为距1970年的时间（{2}），但所填写的导出到SQLite中的格式为时间型的{3}，请声明为SQLite中的数值型", fieldInfo.FieldName, ExcelMethods.GetExcelColumnName(fieldInfo.ColumnSeq + 1), toDatabaseFormatDefine, fieldInfo.DatabaseFieldType);
                        return false;
                    }
                }
            }
            else if (fieldInfo.DataType == DataType.Time)
            {
                ...
                if (fieldInfo.DatabaseFieldType.StartsWith("datetime", StringComparison.CurrentCultureIgnoreCase) || fieldInfo.DatabaseFieldType.Equals("date", StringComparison.CurrentCultureIgnoreCase))
                {
                    errorString = string.Format("time型字段\"{0}\"（列号：{1}）声明导出到SQLite中的数据类型错误，不允许为datetime或date型，如果需要年月日部分，请在Excel中将该字段在本工具中的数据类型改为date型", fieldInfo.FieldName, ExcelMethods.GetExcelColumnName(fieldInfo.ColumnSeq + 1));
                    return false;
                }
                ...
                {
                    errorString = string.Format("time型字段\"{0}\"（列号：{1}）声明导出到SQLite中的形式为距0点的秒数（#sec），但所填写的导出到SQLite中的格式为时间型的time，请声明为SQLite中的数值型", fieldInfo.FieldName, ExcelMethods.GetExcelColumnName(fieldInfo.ColumnSeq + 1));
                    return false;
                }
            }
        ...
        string createTableExtraParam = AppValues.ConfigData.ContainsKey(MySQLStruct.APP_CONFIG_KEY_CREATE_DATABASE_TABLE_EXTRA_PARAM) ? AppValues.ConfigData[MySQLStruct.APP_CONFIG_KEY_CREATE_DATABASE_TABLE_EXTRA_PARAM] : string.Empty;
